...
# Player name and season will be inserted into DB. Modify before running script. 
url = 'https://www.basketball-reference.com/players/l/lillada01/shooting/' + \
    str(season)
...
for shot_div in shots:
    try:
        ...
        top_value = int(style_values[0].split(
            ':')[1].replace('px', '').strip())
        left_value = int(style_values[1].split(
            ':')[1].replace('px', '').strip())
        ...
        print(f"Error processing shot_div: {shot_div}")
        print(f"Exception: {e}")
...
csv_output_path = os.path.join(
    script_dir, '..', 'data', f'{player_name}_{season}_shot_chart_output.csv')
